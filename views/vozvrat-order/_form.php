...
$historyRows = ProductAccountHistory::find()
    ->where(['vozvrat_order_id' => $model->id])
    ->all();
...
// agar modelda product_summ_dollar bo'sh bo'lsa, hisoblangan qiymatni qo'yamiz
if (empty($model->product_summ_dollar)) {
    $model->product_summ_dollar = $initialTotal;
}
...
$isDisabled   = $model->confirmation ? 'disabled' : '';
...
<div class="panel panel-inverse user-index">
  <div class="panel-heading">
    <div class="panel-heading-btn">
      <a href="/vozvrat-order/index class="btn btn-xs btn-warning">
  ...
  <div class="panel-body">
    ...
    ?>
      ...
      <div class="row">
        ...
        <div class="col-md-3">
          <?= $form->field($model, 'old_total_debt')->textInput([
      ...
      <div class="row">
          ...
          <div class="col-md-2">
            <?= $form->field($model, 'product_summ_dollar')->textInput([
                'readonly' => true,
                'style'    => 'margin-left:35px;',
                'id'       => 'product_summ_dollar',
                'value'    => $model->product_summ_dollar,
            ]) ?>
          </div>
      </div>
      ...
      <div class="row">
        <div class="col-md-3">
          <?php
          $sumDollarVal = ($model->all_summ_dollar && $model->all_summ_dollar != 0) ? $model->all_summ_dollar : '';
          echo $form->field($model, 'all_summ_dollar')->textInput([
              'id' => 'all-sum-dollar',
              'value' => $sumDollarVal,
              'required' => true,
      ...
      <div class="row align-items-center">
        ...
        <div class="col-md-3 d-flex justify-content-end">
            <div style="margin-top: 28px;">  <!-- label balandligiga moslash uchun -->
                <?= $form->field($model, 'confirmation', [
                    'options' => ['class' => 'm-0'],                 // field tashqi margin
                    'template' => "{label}\n{input}\n{hint}\n{error}" // label + checkbox yonma-yon
                ])->checkbox([
                    'disabled' => ($model->confirmation == 1),
                    'label' => false,
                    'class' => 'ml-2'
                ])->label("<b style='font-size:16px;color:red;'>Tasdiqlash:</b>", [
...
function recalcTotalSum() {
  ...
  $('#product_summ_dollar').val(total.toFixed(2));
}
